...
public partial class Dispatcher : Node
{
    ...
    [Export] private int _innerCircleRadius = 7;
    [Export] private int _outerCircleRadius = 21;
    [Export] private float _birthInterval1 = 0.278f;
    [Export] private float _birthInterval2 = 0.365f;
    [Export] private float _deathInterval1 = 0.267f;
    [Export] private float _deathInterval2 = 0.445f;
    [Export] private float _alphaN = 0.028f;
    [Export] private float _alphaM = 0.147f;
    [Export] private float _rimWidth = 1.0f;
    ...
    private void CreateAndValidateImages()
    {
        ...
        _inputImage = Image.Create(1024, 1024, false, Image.Format.L8);
        if (_dataTexture is null)
        {
            FastNoiseLite noise = new() { Frequency = .01f };
    ...
    private Rid CreateParametersUniform(int binding)
    {
        int[] intParameters = new[] { _innerCircleRadius, _outerCircleRadius };
        float[] floatParameters = new[]
        {
            _birthInterval1, _birthInterval2, _deathInterval1, _deathInterval2, _alphaM, _alphaN, _rimWidth
        };

        int intBytesCount = intParameters.Length * sizeof(int);
        int floatBytesCount = floatParameters.Length * sizeof(float);
        byte[] bytes = new byte[intBytesCount + floatBytesCount];
        Buffer.BlockCopy(intParameters, 0, bytes, 0, intBytesCount);
        Buffer.BlockCopy(floatParameters, 0, bytes, intBytesCount, floatBytesCount);
    ...
    private void CreateUniforms()
    {
        ...
        _parameters = CreateParametersUniform(2);
